...
%I am unsure why I need to normalize velocities.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Reducetime=1; %Define how many time steps you want to skip.
...
for i=1:size(xmax_final_x,1) %Run through all xmax data
    plot(xmax_final_x(i,:),xmax_final_y(i,:),'-'); %Plot all xmax data
    hold on %Use the same graph
    plot(xmax_final_x(i,size(xmax_final,2)), xmax_final_y(i,size(xmax_final,2)),'.',MarkerSize=30); %Plot final point
    hold on %Use the same graph
end
...
for i=1:size(xmin_final_x,1) %Run through all xmin data
    plot(xmin_final_x(i,:),xmin_final_y(i,:),'-'); %Plot all xmin data
    hold on %Use the same graph
    plot(xmin_final_x(i,size(xmin_final,2)), xmin_final_y(i,size(xmin_final,2)),'.',MarkerSize=30); %Plot final point
    hold on %Use the same graph
end
...
for i=1:size(ymax_final_x,1) %Run through all ymax data
    plot(ymax_final_x(i,:),ymax_final_y(i,:),'-'); %Plot all ymax data
    hold on %Use the same graph
    plot(ymax_final_x(i,size(ymax_final,2)), ymax_final_y(i,size(ymax_final,2)),'.',MarkerSize=30); %Plot final point
    hold on %Use the same graph
end
...
for i=1:size(ymin_final_x,1) %Run through all ymin data
    plot(ymin_final_x(i,:),ymin_final_y(i,:),'-'); %Plot all ymin data
    hold on %Use the same graph
    plot(ymin_final_x(i,size(ymin_final,2)), ymin_final_y(i,size(ymin_final,2)),'.',MarkerSize=30); %Plot final point
    hold on %Use the same graph
end
