...
def draws2data(am, data_draws2data, model_name):
    ...
    #prior_pred.draws_xr(obs_stock_lst).to_netcdf("data/".join(f"{model_name}_PriorPredData.nc"))

    # # plot
    # fig, ax = plt.subplots(figsize=(15, 8))
    # for i in range(len(obs_stock_lst)):
    #     ax.plot(pd.DataFrame(prior_pred.draws_xr(f'{obs_stock_lst[i]}').mean(["chain", "draw"])), label = f"{obs_stock_lst[i]}")
    # ax.legend()
    # plt.show()
    return prior_pred
...
def data2draws(am, data_data2draws, model_name):
    ...
    data2draws_model = cmdstanpy.CmdStanModel(stan_file="stan_files/".join(f"{model_name}_data2draws.stan")) # neg_binom doesn't receive vector; manual add for loop in stanfile
...
vf = VensimFile('vensim_models/Test10.mdl')
...
data_draws2data = {
    "n_t": n_t,
    "inventory_obs": placeholder,
}
print(draws2data(am, data_draws2data, "mngInven").summary())
...
# }
#
# prior_pred = draws2data(am, data_draws2data, "mngInven")
#
# # # print(prior_pred.draws_xr('production_start_rate_stocked_obs'))
# #
# # fig, ax = plt.subplots(figsize=(15, 8))
# # ax.plot(pd.DataFrame(prior_pred.draws_xr('production_start_rate_stocked_obs').mean(["chain", "draw"]).production_start_rate_stocked_obs), label = "production_start_rate_stocked_obs")
# # ax.plot(pd.DataFrame(prior_pred.draws_xr('production_rate_stocked_obs').mean(["chain", "draw"]).production_rate_stocked_obs), label = "production_rate_stocked_obs")
