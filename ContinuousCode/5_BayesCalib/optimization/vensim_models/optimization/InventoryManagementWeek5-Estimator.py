"""
Python model 'InventoryManagementWeek5-Estimator.py'
...
@component.add(
    ...
    comp_type="Stateful",
    comp_subtype="Integ",
    depends_on={"_integ_cumulative_payoff": 1},
    other_deps={
        "_integ_cumulative_payoff": {"initial": {}, "step": {"calib_payoff": 1}}
    },
)
def cumulative_payoff():
    return _integ_cumulative_payoff()


_integ_cumulative_payoff = Integ(
    lambda: calib_payoff(), lambda: 0, "_integ_cumulative_payoff"
)
...
def customer_order_rate():
    return initial_customer_order_rate()


@component.add(
    name="Min Err", units="Dimensionless", comp_type="Constant", comp_subtype="Normal"
)
def min_err():
    return 100
...
@component.add(
    ...
    comp_subtype="Normal",
    depends_on={
        "production_rate_err": 1,
        "production_start_rate_err": 1,
        "shipment_rate_err": 1,
    },
)
def calib_payoff():
    return production_rate_err() + production_start_rate_err() + shipment_rate_err()
...
@component.add(
    name="Production Rate Err",
    units="Dimensionless",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
        "production_rate_measured_data": 2,
        "production_rate": 1,
        "production_measurement_noise_scale": 1,
        "min_err": 1,
    },
)
def production_rate_err():
    return (production_rate_measured_data() - production_rate()) ** 2 / np.maximum(
        min_err(),
        production_measurement_noise_scale() * production_rate_measured_data(),
    ) ** 2


@component.add(
    name="Production Start Rate Err",
    units="dmnl",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
        "production_start_rate_measured_data": 2,
        "production_start_rate": 1,
        "min_err": 1,
        "production_start_measurement_noise_scale": 1,
    },
)
def production_start_rate_err():
    return (
        production_start_rate_measured_data() - production_start_rate()
    ) ** 2 / np.maximum(
        min_err(),
        production_start_measurement_noise_scale()
        * production_start_rate_measured_data(),
    ) ** 2


@component.add(
    name="Shipment Rate Err",
    units="Dimensionless",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
        "shipment_rate_measured_data": 2,
        "shipment_rate": 1,
        "shipment_measurement_noise_scale": 1,
        "min_err": 1,
    },
)
def shipment_rate_err():
    return (shipment_rate_measured_data() - shipment_rate()) ** 2 / np.maximum(
        min_err(), shipment_measurement_noise_scale() * shipment_rate_measured_data()
    ) ** 2
...
def shipment_rate_measured_data():
    return (
        stats.truncnorm.rvs(
            0, 2, loc=1, scale=shipment_measurement_noise_scale(), size=()
        )
        * shipment_rate()
    )
...
@component.add(
    name="Shipment Measurement Noise Scale",
    units="dmnl",
    comp_type="Constant",
    comp_subtype="Normal",
)
def shipment_measurement_noise_scale():
    return 0.1


@component.add(
    name="Production Start Measurement Noise Scale",
    units="dmnl",
    comp_type="Constant",
    comp_subtype="Normal",
)
def production_start_measurement_noise_scale():
    return 0.1
...
def production_start_rate_measured_data():
    return (
        stats.truncnorm.rvs(
            0, 2, loc=1, scale=production_start_measurement_noise_scale(), size=()
        )
...
@component.add(
    name="Production Measurement Noise Scale",
    units="dmnl",
    comp_type="Constant",
    comp_subtype="Normal",
)
def production_measurement_noise_scale():
    return 0.1
...
def production_rate_measured_data():
    return (
        np.exp(
            production_measurement_noise_scale()
