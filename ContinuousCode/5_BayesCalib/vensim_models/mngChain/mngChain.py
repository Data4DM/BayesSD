...
@component.add(
    name="Production Start Rate Measured",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
def production_start_rate_measured():
    return (
        stats.truncnorm.rvs(
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"white_noise": 1, "process_noise": 1, "noise_correlation_time": 1},
)
def change_in_process_noise():
    """
    Change in the process noise value; process noise is a first order exponential smoothing delay of the white noise input.
    """
    return (white_noise() - process_noise()) / noise_correlation_time()
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"input_1": 1, "initial_customer_order_rate": 1},
)
def customer_order_rate():
    return np.maximum(0, input_1() * initial_customer_order_rate())
...
def input_1():
    ...
    return (
        ...
        + (pulse_quantity() / pulse_duration())
        * pulse(__data["time"], pulse_time(), width=pulse_duration())
        + ramp(__data["time"], ramp_slope(), ramp_start_time(), ramp_end_time())
        + sine_amplitude() * np.sin(2 * 3.14159 * time() / sine_period())
        + step(__data["time"], 1, noise_start_time()) * process_noise()
    )
...
@component.add(
    name="Noise Start Time", units="Weeks", comp_type="Constant", comp_subtype="Normal"
)
def noise_start_time():
    """
...
@component.add(
    name="Pulse Duration", units="Week", comp_type="Constant", comp_subtype="Normal"
)
def pulse_duration():
    """
...
def pulse_quantity():
    """
    The quantity to be injected to customer orders, as a fraction of the base value of Input. For example, to pulse in a quantity equal to 50% of the current value of input, set to .50.
    """
    return 0


@component.add(
    name="Pulse Time", units="Year", comp_type="Constant", comp_subtype="Normal"
)
def pulse_time():
    """
...
@component.add(
    name="Ramp End Time", units="Year", comp_type="Constant", comp_subtype="Normal"
)
def ramp_end_time():
    """
...
@component.add(
    name="Ramp Slope", units="1/Year", comp_type="Constant", comp_subtype="Normal"
)
def ramp_slope():
    """
...
@component.add(
    name="Ramp Start Time", units="Week", comp_type="Constant", comp_subtype="Normal"
)
def ramp_start_time():
    """
...
@component.add(
    name="Sine Period", units="Week", comp_type="Constant", comp_subtype="Normal"
)
def sine_period():
    """
    Period of sine wave in customer demand. Set initially to 4 years to simulate the business cycle
    """
    return 4
...
@component.add(
    name="Step Time", units="Week", comp_type="Constant", comp_subtype="Normal"
)
def step_time():
    """
