...
real dataFunc__process_noise_uniform_driving(real time, real time_step){
    ...
    if(time <= time_step * 1){
        intercept = -0.4950320201045566;
        real local_time_step = time_step * 1 - time_step * 0;
        slope = (0.21012686351115661 - -0.4950320201045566) / local_time_step;
        return intercept + slope * (time - time_step * 0);
    }
    else if(time <= time_step * 2){
        intercept = 0.21012686351115661;
        real local_time_step = time_step * 2 - time_step * 1;
        slope = (0.37839440191040474 - 0.21012686351115661) / local_time_step;
        return intercept + slope * (time - time_step * 1);
    }
    return 0.37839440191040474;
}
